...
export function ComprehensiveFarmerForm({ formData, setFormData, errors, mode = 'create', existingFarms = [] }: FarmerFormProps) {
    ...
    const [farmCreated, setFarmCreated] = React.useState(false);
    const [showNewFarmCard, setShowNewFarmCard] = React.useState(false);
    const [showFarmSelection, setShowFarmSelection] = React.useState(false);
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                {farmCreated 
                                    ? "Select a farm to add land parcels"
                                    : "Create a farm first, then add land parcels to it"
                                }
                            </CardDescription>
                        </div>
                    </div>
                ...
                <CardContent className="grid gap-6">
                    {/* Initial State - Show only Add Farm button */}
                    {!showFarmCreation && !farmCreated && (
                        <div className="text-center py-8">
                            ...
                            <p className="text-muted-foreground mb-4 max-w-md mx-auto">
                                {existingFarms && existingFarms.length > 0
                                    ? "You have existing farms. Create a new farm or select an existing one to add parcels."
                                    : "Create your first farm to get started with adding land parcels."
                                }
                            </p>
                            <Button
                                type="button"
                                onClick={() => setShowFarmCreation(true)}
                                className="gap-2"
                            >
                                + Add New Farm
                            </Button>
                    ...
                    {showFarmCreation && !farmCreated && (
                        <Card className="border-2 bg-muted/30">
                            <CardHeader>
                                <CardTitle className="text-base">Create New Farm</CardTitle>
                                <CardDescription>Enter farm details below</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        ...
                                        disabled={!formData.farm_name || formData.farm_name.trim() === ''}
                                    >
                                        Save Farm
                                    </Button>
                                    <Button
                                        ...
                                        onClick={() => {
                                            setShowFarmCreation(false);
                                            setFormData({ ...formData, farm_name: '', existing_farm_id: '' });
                                        }}
                                    >
                                        Cancel
                                    </Button>
                    ...
                    {/* Farm Parcels - Only show if farm exists */}
                    {formData.farm_name && formData.farm_name.trim() !== '' ? (
                        <div className="space-y-4">
                            <div className="flex items-center justify-between border-b pb-2">
                                <h4 className="text-sm font-semibold">Farm Parcels</h4>
                                <Button
                                    type="button"
                                    variant="outline"
                                    size="sm"
                                    onClick={() => {
                                        const newParcels = [...(formData.farm_parcels || []), {
                                            parcel_number: '',
                                            barangay: '',
                                            city_municipality: '',
                                            total_farm_area: '',
                                        ...
                                        setFormData({ ...formData, farm_parcels: newParcels });
                                    }}
                                >
                                    + Add Farm Parcel
                                </Button>
                        ...
                        {formData.farm_parcels?.map((parcel: any, index: number) => (
                            <div key={index} className="p-4 border rounded-lg space-y-4 relative bg-muted/30">
                                <div className="flex items-center justify-between">
                                    <h5 className="font-medium">Parcel #{index + 1}</h5>
                                    {formData.farm_parcels.length > 1 && (
                                        <button
                                            type="button"
                                            onClick={() => {
                                                const newParcels = formData.farm_parcels.filter((_: any, i: number) => i !== index);
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }}
                                            className="text-red-600 hover:text-red-800 text-sm"
                                        >
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].parcel_number = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].barangay = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].city_municipality = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].total_farm_area = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].parcel_size = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].livestock_count = parseInt(e.target.value) || 0;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="grid gap-2">
                                        ...
                                        <Select 
                                            ...
                                            onValueChange={(value) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].farm_type = value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }}
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Select 
                                            ...
                                            onValueChange={(value) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].ownership_type = value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }}
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].landowner_name = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                            placeholder="If not owner"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].ownership_document_type = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].ownership_document_number = e.target.value;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                    </div>
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="flex items-center space-x-2">
                                        <Checkbox 
                                            id={`within_ancestral_domain_${index}`} 
                                            checked={parcel.within_ancestral_domain || false} 
                                            onCheckedChange={(checked: boolean) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].within_ancestral_domain = checked;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                        <Label htmlFor={`within_ancestral_domain_${index}`}>Within Ancestral Domain</Label>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <Checkbox 
                                            id={`agrarian_reform_${index}`} 
                                            checked={parcel.agrarian_reform_beneficiary || false} 
                                            onCheckedChange={(checked: boolean) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].agrarian_reform_beneficiary = checked;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                        <Label htmlFor={`agrarian_reform_${index}`}>Agrarian Reform Beneficiary</Label>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <Checkbox 
                                            id={`is_organic_${index}`} 
                                            checked={parcel.is_organic_practitioner || false} 
                                            onCheckedChange={(checked: boolean) => {
                                                const newParcels = [...formData.farm_parcels];
                                                newParcels[index].is_organic_practitioner = checked;
                                                setFormData({ ...formData, farm_parcels: newParcels });
                                            }} 
                                        />
                                        <Label htmlFor={`is_organic_${index}`}>Organic Practitioner</Label>
                                    </div>
                                </div>
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Input 
                                        ...
                                        onChange={(e) => {
                                            const newParcels = [...formData.farm_parcels];
                                            newParcels[index].remarks = e.target.value;
                                            setFormData({ ...formData, farm_parcels: newParcels });
                                        }} 
                        ...
                        <div className="p-4 border rounded-lg bg-muted/30 text-center">
                            <p className="text-muted-foreground">Please enter a farm name first to add farm parcels</p>
                        </div>
                    )}
                </CardContent>
            </Card>
