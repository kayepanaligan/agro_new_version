...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Eligibility Rules', href: '/admin/eligibility-rules' }];
...
export default function EligibilityRules() {
    ...
    const handleCreate = () => { router.post('/admin/eligibility-rules', formData, { preserveScroll: false, onSuccess: () => { setIsCreateModalOpen(false); resetForm(); } }); };
    const handleUpdate = () => { if (!selectedRule) return; router.put(`/admin/eligibility-rules/${selectedRule.id}`, formData, { preserveScroll: false, onSuccess: () => { setIsEditModalOpen(false); resetForm(); } }); };
    const handleDelete = () => { if (!selectedRule) return; router.delete(`/admin/eligibility-rules/${selectedRule.id}`, { preserveScroll: false, onSuccess: () => { setIsDeleteModalOpen(false); setSelectedRule(null); } }); };
