        ...
        <SidebarContext.Provider value={contextValue}>
            <TooltipProvider delayDuration={0}>
                <div
                    style={
                        {
                            '--sidebar-width': customWidth && sidebarMode === 'expanded' ? `${customWidth}px` : SIDEBAR_WIDTH,
        ...
        >
            ...
            <div
                className={cn(
                    ...
                    // Hidden state: slide off-screen and collapse width
                    side === 'left'
                        ? 'group-data-[collapsible=hidden]:left-[calc(var(--sidebar-width)*-1)]'
                        : 'group-data-[collapsible=hidden]:right-[calc(var(--sidebar-width)*-1)]',
                    'group-data-[collapsible=hidden]:w-0 group-data-[collapsible=hidden]:opacity-0 group-data-[collapsible=hidden]:pointer-events-none',
...
const SidebarResizeHandle = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(({ className, ...props }, ref) => {
    ...
    const liveWidthRef = React.useRef(customWidth ?? SIDEBAR_DEFAULT_WIDTH);

    // Keep ref in sync with latest customWidth
    React.useEffect(() => {
        liveWidthRef.current = customWidth ?? SIDEBAR_DEFAULT_WIDTH;
    }, [customWidth]);

    const handleMouseDown = React.useCallback(
        (e: React.MouseEvent) => {
            e.preventDefault();
            const currentWidth = customWidth ?? SIDEBAR_DEFAULT_WIDTH;
            startXRef.current = e.clientX;
            startWidthRef.current = currentWidth;
            setIsResizing(true);

            // Ensure expanded mode during resize
            if (state !== 'expanded') {
                setSidebarMode('expanded');
            }
        ...
        [customWidth, setIsResizing, setSidebarMode, state],
    );
    ...
    React.useEffect(() => {
        ...
        const handleMouseUp = () => {
            ...
            if (finalWidth < SIDEBAR_HIDE_THRESHOLD) {
                setSidebarMode('hidden');
                setCustomWidth(null);
            } else if (finalWidth < SIDEBAR_COLLAPSE_THRESHOLD) {
...
const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(({ className, ...props }, ref) => {
    return (
        <div
            ...
            className={cn('flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-visible', className)}
            {...props}
        />
    );
});
...
const sidebarMenuButtonVariants = cva(
    'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 group-data-[collapsible=icon]:pointer-events-auto!',
    {
        variants: {
            variant: {
